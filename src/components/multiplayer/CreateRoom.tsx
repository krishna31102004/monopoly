...
export function CreateRoom() {
  ...
  const [name, setName] = useState(""); const [token, setToken] = useState<PlayerToken | null>(null); const [tokenLabel, setTokenLabel] = useState(""); const [tokenColor, setTokenColor] = useState(""); const [nameError, setNameError] = useState(""); const [submitting, setSubmitting] = useState(false); const [sawPreGame, setSawPreGame] = useState(false);
  useEffect(() => { if (error) setSubmitting(false); }, [error]);
  useEffect(() => { if (room?.status === "lobby" || room?.status === "rollOff") setSawPreGame(true); }, [room?.status]);
  ...
  if (room && myPlayerId && gameState && room.status === "inGame") return <GameLayoutMultiplayer gameState={gameState} myPlayerId={myPlayerId} room={room} sendAction={sendAction} error={error} connectionStatus={status} onLeave={leaveRoom} onForfeit={forfeitGame} onRequestSync={requestGameSync} tradeDraft={tradeDraft} startTradeDraft={startTradeDraft} updateTradeDraft={updateTradeDraft} cancelTradeDraft={cancelTradeDraft} submitTradeDraft={submitTradeDraft} showStartSequence={sawPreGame} onStartSequenceShown={() => setSawPreGame(false)} />;
