...
import type { HiddenAuctionFinalBid } from "@/types/game";
...
export function HiddenAuctionPanel({ state, dispatch, myPlayerId, ownBid = null, serverAuthoritative = false }: Props) {
  ...
  const tiedPlayers = auction.result?.kind === "tie"
    ...
    : [];
  const finalBids = auction.result?.kind === "winner" || auction.result?.kind === "no-bid"
    ? auction.result.finalBids
    : [];
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-end justify-center bg-slate-950/85 p-0 backdrop-blur-sm sm:items-center sm:p-3" role="presentation">
      ...
      >
        ...
        </div> : <HiddenAuctionResult winner={winner ?? undefined} spaceName={space.name} winningBid={auction.result?.winningBid ?? 0} tiedPlayers={tiedPlayers} tieBid={auction.result?.kind === "tie" ? auction.result.winningBid : 0} finalBids={finalBids} revealStep={revealStep} />}
...
function HiddenAuctionResult({ winner, spaceName, winningBid, tiedPlayers, tieBid, finalBids, revealStep }: { winner: GameState["players"][number] | undefined; spaceName: string; winningBid: number; tiedPlayers: string[]; tieBid: number; finalBids: HiddenAuctionFinalBid[]; revealStep: 1 | 2 | 3 | null }) {
  ...
  return <div className="flex min-h-[340px] flex-1 flex-col items-center justify-center overflow-y-auto p-5 text-center"><div className="w-full max-w-md rounded-[var(--wc-radius-large)] border border-[var(--wc-gold-border)] bg-[var(--wc-navy-raised)] p-5">{winner ? <><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-300">Hidden auction result</p><p className="mt-2 text-2xl font-black text-white">{winner.name} wins {spaceName}!</p><p className="wc-numeric mt-2 text-3xl font-black text-[var(--wc-gold)]">${winningBid}</p><HiddenAuctionFinalBids bids={finalBids} /></> : tiedPlayers.length > 0 ? <><p className="text-xs font-black uppercase tracking-[0.2em] text-[var(--wc-gold)]">{tiedPlayers.length}-way tie</p><p className="mt-2 text-2xl font-black text-white">{tiedPlayers.join(" and ")}</p><p className="wc-numeric mt-2 text-xl font-black text-[var(--wc-gold)]">${tieBid} each</p><p className="mt-2 text-sm text-slate-300">Tie-break auction starting with only the tied players.</p></> : <><p className="text-xs font-black uppercase tracking-[0.2em] text-slate-300">Hidden auction result</p><p className="mt-2 text-2xl font-black text-white">No winning bid</p><p className="mt-2 text-slate-300">{spaceName} remains unowned.</p><HiddenAuctionFinalBids bids={finalBids} /></>}</div></div>;
}

function HiddenAuctionFinalBids({ bids }: { bids: HiddenAuctionFinalBid[] }) {
  if (bids.length === 0) return null;
  return <section className="mt-5 border-t border-slate-700 pt-3 text-left" aria-label="Final bids"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Final bids</p><ol className="mt-2 space-y-1.5">{bids.map((bid, index) => <li key={bid.playerId} className={`flex items-center gap-2 rounded px-2 py-1.5 text-sm ${bid.isWinner ? "border border-emerald-500/50 bg-emerald-950/45" : "bg-slate-900/60"}`}><span className="wc-numeric w-4 shrink-0 text-xs font-black text-slate-500">{index + 1}.</span><span className={`min-w-0 flex-1 truncate font-bold ${bid.isWinner ? "text-emerald-200" : "text-slate-200"}`}>{bid.playerName}{bid.isWinner ? <span className="ml-1 text-[10px] uppercase tracking-wide text-emerald-300">Winner</span> : null}</span><span className={`wc-numeric shrink-0 font-black ${bid.isWinner ? "text-emerald-200" : "text-white"}`}>{bid.amount > 0 ? `$${bid.amount}` : "No bid"}</span></li>)}</ol></section>;
}
